...
def save_result(args, results, command_string):
    ...
    if not os.path.exists(filename):
        with open(f"{filename}", 'a+') as write_obj:
            write_obj.write("method, weight_decay, dropout, lr, epochs, num_heads, num_layers, rb_order, M, K, lamda, hidden_channels, test_acc, command \n")
    with open(f"{filename}", 'a+') as write_obj:
        write_obj.write(
            f"{args.method}, " + f"{args.weight_decay}, " + f"{args.dropout}, " + f"{args.lr}, " + \
            f"{args.epochs}, " + f"{args.num_heads}, " + f"{args.num_layers}, " + \
            f"{args.hidden_channels}, " + f"{args.training_hops}, " + \
            f"{results.mean():.2f} $\pm$ {results.std():.2f}, " + f"{command_string} \n")
