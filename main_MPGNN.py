...
if __name__ == '__main__':
    ...
    if args.dataset == 'ogbn-papers100M':
        ...
        split_idx_lst = data_dict['splits']
        train_idx = split_idx_lst['train']
        val_idx = split_idx_lst['valid']
        ...
        for col_idx in range(args.col_split):
            ...
            train_data = read_file(hop0_train_path, (split_idx_lst['train'].shape[0], in_dim//args.col_split))
            val_data = read_file(hop0_val_path, (split_idx_lst['valid'].shape[0], in_dim//args.col_split))
            test_data = read_file(hop0_test_path, (split_idx_lst['test'].shape[0], in_dim//args.col_split))
    ...
    now = datetime.now()
    # Convert it to a string in the format year-month-day_hour-minute-second
    ...
    for run in range(args.runs):
        ...
        print(f"val_idx size: {len(split_idx['valid'])}")
        print(f"test_idx size: {len(split_idx['test'])}")
        ...
        print(f"Total parameters: {total_params}, Total grad parameters: {total_grad_params}")
        ...
        with conditional_profile(args.enable_profiling, 'DGL_SINGLE') as p:
            for epoch in range(args.epochs):
                ...
                for it, (input_nodes, output_nodes, blocks) in enumerate(
                    ...
                    total_source_nodes += blocks[0].number_of_src_nodes()
                    total_source_features += x.shape[0] * x.shape[1]                        
                ...
                # print(f'total_transfer_time: {total_transfer_time:.4f} sec, proportion: {total_transfer_time/(train_time/1000):.4f}')
                # print(f'number of source nodes: {total_source_nodes/1e6:.4f} M, size of source features: {4*total_source_features/1e6:.4f} MB')
                if epoch > args.test_start_epoch and epoch % args.eval_step == 0:
                    ...
                    if args.dataset in ('ogbn-papers100M'):
                        result = evaluate_sample(model, g, split_idx, eval_func, criterion, eval_device, c, args)
                    else:
                        result = evaluate_batch(model, g, split_idx, eval_func, criterion, eval_device, args)
